...
public class SummaryService {
    ...
    public static void update(Summary summary) {
        String sql = "update  purchase set totalqty=?, availableqty=? soldQty=? where code=?)";
        try {
            PreparedStatement ps = con.prepareStatement(sql);

            ps.setInt(1, summary.getTotalQty());
            ps.setInt(2, summary.getAvailableQty());
            ps.setInt(3, summary.getSoldQty());
            ps.setString(4, summary.getCode());
    ...
    public static List<Summary> insert() {
